        ...
        loop {
            ...
            if should_shutdown {
                ...
                for (cid, tx) in companions.iter_mut() {
                    ...
                    let _ = tx.try_send(ServerToCompanion::Command { target_id: None, cmd: "shutdown".into() });
                }
...
pub async fn broadcast_node_status(state: &AppState) {
    ...
    for (_, dash_tx) in dashboards.iter_mut() {
        let _ = dash_tx.send(ServerToDashboard::NodeStatus {
            online_count: c_len,
            shutdown_deadline,
...
pub async fn check_and_update_global_deadline(state: &AppState, force_reset: bool) {
    ...
    for (id, scripts) in records {
        if online_companions.contains_key(&id) {
            let parsed: HashMap<String, String> = serde_json::from_str(&scripts).unwrap_or_default();
            if !parsed.is_empty() {
                has_probes = true;
                break;
            }
    ...
    let minutes_record: Option<(String,)> = sqlx::query_as("SELECT value FROM system_config WHERE key = 'auto_shutdown_minutes'")
        .fetch_optional(&state.db)
        .await
        .unwrap_or(None);
        
    let minutes = minutes_record.and_then(|(v,)| v.parse::<u32>().ok()).unwrap_or(0);
    ...
    if force_reset || deadline_lock.is_none() {
        *deadline_lock = Some(std::time::Instant::now() + std::time::Duration::from_secs((minutes * 60) as u64));
        drop(deadline_lock);
        drop(online_companions);
        broadcast_node_status(state).await;
    }
                        ...
                        if let Ok(parsed) = serde_json::from_str::<CompanionToServer>(&line) {
                            match parsed {
                                CompanionToServer::Hello { client_id } => {
                                    let record: Option<(String,)> = sqlx::query_as("SELECT scripts FROM companions WHERE id = ?")
                                        .bind(&client_id)
                                    ...
                                    match record {
                                        Some((scripts_json,)) => {
                                            ...
                                            state_clone.companions.lock().await.insert(client_id.clone(), tx.clone());
                                            
                                            // Flush pending commands
                                            {
                                                let mut pc = state_clone.pending_commands.lock().await;
                                                let now = std::time::Instant::now();
                                                pc.retain(|(t, _)| now.duration_since(*t).as_secs() < 60);
                                                
                                                let mut to_remove = Vec::new();
                                                for (i, (_, cmd_msg)) in pc.iter().enumerate() {
                                                    let mut should_send = false;
                                                    if let ServerToCompanion::Command { target_id, .. } = cmd_msg {
                                                        if target_id.is_none() || target_id.as_ref() == Some(&client_id) {
                                                            should_send = true;
                                                        }
                                                    }
                                                    if should_send {
                                                        if let ServerToCompanion::Command { cmd, .. } = cmd_msg {
                                                            info!("Flushing pending command {} to newly connected companion {}", cmd, client_id);
                                                            let _ = tx.try_send(ServerToCompanion::Command { target_id: None, cmd: cmd.clone() });
                                                            to_remove.push(i);
                                                        }
                                                    }
                                                }
                                            ...
                                            info!("Sending ConfigUpdate to {} with {} scripts", client_id, scripts.len());
                                            let _ = tx.try_send(ServerToCompanion::ConfigUpdate { scripts });

                                            broadcast_node_status(&state_clone).await;
                                            check_and_update_global_deadline(&state_clone, false).await;
                                        }
                                        None => {
                                            warn!("QUIC Client rejected: UUID not found in companions table: {}", client_id);
                                            break;
                                        }
                                    }
                                }
                                CompanionToServer::Status { client_id, active, active_service, probe_type } => {
                                    if let Some(ref cid) = client_id_opt {
                                        if &client_id == cid {
                                            info!("QUIC Status from {}: probe={}, active={}, service={:?}", cid, probe_type, active, active_service);
                                            let mut procs = state_clone.active_services.lock().await;
                                            let client_procs = procs.entry(cid.clone()).or_insert_with(std::collections::HashMap::new);
                                            if active {
                                                if probe_type != "heartbeat" {
                                                    if let Some(p) = &active_service {
                                                        client_procs.insert(probe_type.clone(), p.clone());
                                                    } else {
                                                        client_procs.insert(probe_type.clone(), "Unknown".to_string());
                                                    }
                                                    let _ = client_procs;
                                                    drop(procs);
                                                    check_and_update_global_deadline(&state_clone, true).await;
                                                }
                                            } else {
                                                client_procs.remove(&probe_type);
                                            }
                                            
                                            let mut dashboards = state_clone.dashboards.lock().await;
                                            for (_, dash_tx) in dashboards.iter_mut() {
                                                let _ = dash_tx.try_send(ServerToDashboard::Status {
                                                    client_id: cid.clone(),
                                                    active,
