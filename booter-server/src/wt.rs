        ...
        match session.accept_bi().await {
            Ok((mut send_stream, recv_stream)) => {
                ...
                tokio::spawn(async move {
                    ...
                    let forbidden_record: Option<(String,)> = sqlx::query_as(
                        "SELECT value FROM system_config WHERE key = 'boot_forbidden_time'",
                    )
                    ...
                    let cooldown_record: Option<(String,)> = sqlx::query_as(
                        "SELECT value FROM system_config WHERE key = 'boot_cooldown_minutes'",
                    )
                    .fetch_optional(&state.db)
                    .await
                    .unwrap_or(None);
                        ...
                        tokio::spawn(async move {
                            loop {
                                ...
                                match buf_reader.read_line(&mut line).await {
                                    ...
                                    Ok(_) => {
                                        if let Ok(msg) =
                                            serde_json::from_str::<DashboardToServer>(&line)
                                        {
                                            match msg {
                                                DashboardToServer::Command { target_id, cmd } => {
                                                    if cmd != "shutdown" {
                                                        warn!(
                                                            "Dashboard {} requested unsupported command: {}",
                                                            dash_id, cmd
                                                        );
                                                        let _ = tx
                                                            .send(
                                                                ServerToDashboard::CommandResult {
                                                                    success: false,
                                                    ...
                                                    if cmd == "shutdown" {
                                                        let active_services =
                                                            state.active_services.lock().await;
                                                        let has_active =
                                                        ...
                                                        if has_active {
                                                            let _ = tx
                                                                .send(ServerToDashboard::CommandResult {
                                                                    success: false,
                                                                    message: "当前有服务在线，拒绝执行关机指令"
                                                                        .into(),
                                                                })
                                                                .await;
                                                            continue;
                            ...
                            while let Some(msg) = rx.recv().await {
                                if let Ok(mut encoded) = serde_json::to_vec(&msg) {
                                    ...
                                    if let Err(e) = send_stream.write_all(&encoded).await {
                                        warn!(
                                            "Failed to write msg to dashboard {}: {}",
                                            dash_id, e
                                        );
                                        break;
                                    }
